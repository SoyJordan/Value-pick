...
["GOLES","home05","Local Over 0.5"],["GOLES","homeU05","Local Under 0.5"],
["GOLES","home15","Local Over 1.5"],["GOLES","homeU15","Local Under 1.5"],
["GOLES","home25","Local Over 2.5"],["GOLES","homeU25","Local Under 2.5"],
["GOLES","away05","Visitante Over 0.5"],["GOLES","awayU05","Visitante Under 0.5"],
["GOLES","away15","Visitante Over 1.5"],["GOLES","awayU15","Visitante Under 1.5"],
["GOLES","away25","Visitante Over 2.5"],["GOLES","awayU25","Visitante Under 2.5"],
...
function applyQuickOdds(){
 const raw=document.getElementById('quickOdds')?.value||'';const map={'1':'home','x':'draw','2':'away','1dnb':'homeDnb','dnb1':'homeDnb','2dnb':'awayDnb','dnb2':'awayDnb','1x':'homeOrDraw','x2':'awayOrDraw','o1.5':'over15','over1.5':'over15','u1.5':'under15','under1.5':'under15','o2.5':'over25','over2.5':'over25','u2.5':'under25','under2.5':'under25','o3.5':'over35','over3.5':'over35','u3.5':'under35','under3.5':'under35','localo0.5':'home05','localover0.5':'home05','localu0.5':'homeU05','localunder0.5':'homeU05','localo1.5':'home15','localover1.5':'home15','localu1.5':'homeU15','localunder1.5':'homeU15','localo2.5':'home25','localover2.5':'home25','localu2.5':'homeU25','localunder2.5':'homeU25','visitanteo0.5':'away05','visitanteover0.5':'away05','visitanteu0.5':'awayU05','visitanteunder0.5':'awayU05','visitanteo1.5':'away15','visitanteover1.5':'away15','visitanteu1.5':'awayU15','visitanteunder1.5':'awayU15','visitanteo2.5':'away25','visitanteover2.5':'away25','visitanteu2.5':'awayU25','visitanteunder2.5':'awayU25','bttssi':'bttsYes','bttsyes':'bttsYes','bttsno':'bttsNo'};
...
<b>+1 al Local (equivale -1 al Visitante)</b>
<div class="grid" style="margin-top:6px">
<label>1 · Local +1<input data-eh-line="1" data-eh-outcome="home" type="number" min="1.01" step=".01" placeholder="Cuota"></label>
<label>X · Empate (+1 al Local)<input data-eh-line="1" data-eh-outcome="draw" type="number" min="1.01" step=".01" placeholder="Cuota"></label>
...
// V2.0 · las dos vías de bajas (ataque propio + defensa rival) se combinan con tope,
...
const h0=pois(0,hl),h1=pois(1,hl),h2=pois(2,hl),a0=pois(0,al),a1=pois(1,al),a2=pois(2,al);
p.home05=1-h0;p.homeU05=h0;p.home15=1-(h0+h1);p.homeU15=h0+h1;p.home25=1-(h0+h1+h2);p.homeU25=h0+h1+h2;
p.away05=1-a0;p.awayU05=a0;p.away15=1-(a0+a1);p.awayU15=a0+a1;p.away25=1-(a0+a1+a2);p.awayU25=a0+a1+a2;
return p;
}
...
function europeanHandicapLabel(line,outcome){
 ...
 return `H1X2 · Empate (${fmt(homeLine)} al Local)`;
}
...
function lambdaContradiction(row,opts={}){
 ...
 if(!opposes)return {gap,penalty:0,opposes:false};
 // Build 2000: la dirección deportiva del λ manda antes que el precio.
 // El EV sigue visible, pero una cuota alta no convierte por sí sola al lado contrario en VALUE BET.
 const penalty=gap>=.15?clamp((gap-.15)*30+5,5,25):clamp(gap*20,0,3);
 return {gap,penalty,opposes:true};
}
function marketLambdaCoherence(row,opts={}){
 const k=String(row.market||""),hl=Number(opts.hl),al=Number(opts.al);
 if(!Number.isFinite(hl)||!Number.isFinite(al))return {penalty:0,maxDecision:null,reason:"",structural:false};
 const total=hl+al,minL=Math.min(hl,al);
 const conf=Number(row.selectorConf??row.conf)||0,rob=Number(row.robustness)||0,worst=Number(row.worstEV)||0;
 const exceptional=(reqWorst=.10)=>conf>=85&&rob>=80&&worst>=reqWorst;
 const cap=(maxDecision,reason,penalty)=>({penalty,maxDecision,reason,structural:true});

 // Build 2000: reglas propias de coherencia para GOLES y BTTS.
 // La cuota puede revelar valor, pero no puede ignorar una proyección λ estructuralmente incompatible.
 if(k==="home05"||k==="away05"){
   const teamL=k==="home05"?hl:al;
   if(teamL<.60)return cap("NO BET",`λ del equipo ${teamL.toFixed(2)} demasiado bajo para Over 0.5`,20);
   if(teamL<.80&&!exceptional(.08))return cap("WATCH",`λ del equipo ${teamL.toFixed(2)} bajo para Over 0.5: máximo WATCH`,8);
 }
 if(k==="home15"||k==="away15"){
   const teamL=k==="home15"?hl:al;
   if(teamL<1.00)return cap("NO BET",`λ del equipo ${teamL.toFixed(2)} incompatible con Over 1.5`,24);
   if(teamL<1.25&&!exceptional(.10))return cap("WATCH",`λ del equipo ${teamL.toFixed(2)} débil para Over 1.5: máximo WATCH`,10);
 }
 if(k==="home25"||k==="away25"){
   const teamL=k==="home25"?hl:al;
   if(teamL<1.35)return cap("NO BET",`λ del equipo ${teamL.toFixed(2)} incompatible con Over 2.5`,28);
   if(teamL<1.70&&!exceptional(.14))return cap("WATCH",`λ del equipo ${teamL.toFixed(2)} débil para Over 2.5: máximo WATCH`,14);
 }
 if(["homeU05","awayU05"].includes(k)){
   const teamL=k.startsWith("home")?hl:al;if(teamL>1.15)return cap("NO BET",`λ del equipo ${teamL.toFixed(2)} alto para Under 0.5`,24);
 }
 if(["homeU15","awayU15"].includes(k)){
   const teamL=k.startsWith("home")?hl:al;if(teamL>2.00)return cap("NO BET",`λ del equipo ${teamL.toFixed(2)} alto para Under 1.5`,22);if(teamL>1.65&&!exceptional(.10))return cap("WATCH",`λ del equipo ${teamL.toFixed(2)} alto para Under 1.5`,9);
 }
 if(["homeU25","awayU25"].includes(k)){
   const teamL=k.startsWith("home")?hl:al;if(teamL>2.80)return cap("NO BET",`λ del equipo ${teamL.toFixed(2)} alto para Under 2.5`,22);if(teamL>2.30&&!exceptional(.10))return cap("WATCH",`λ del equipo ${teamL.toFixed(2)} alto para Under 2.5`,9);
 }
 if(k==="over15"){
   if(total<1.75)return cap("NO BET",`λ total ${total.toFixed(2)} demasiado bajo para Over 1.5`,20);
   if(total<2.05&&!exceptional(.08))return cap("WATCH",`λ total ${total.toFixed(2)} bajo para Over 1.5: máximo WATCH`,7);
 }
 if(k==="over25"){
   if(total<2.20)return cap("NO BET",`λ total ${total.toFixed(2)} incompatible con Over 2.5`,24);
   if(total<2.55&&!exceptional(.10))return cap("WATCH",`λ total ${total.toFixed(2)} débil para Over 2.5: máximo WATCH`,10);
 }
 if(k==="over35"){
   if(total<2.80)return cap("NO BET",`λ total ${total.toFixed(2)} incompatible con Over 3.5`,26);
   if(total<3.25&&!exceptional(.12))return cap("WATCH",`λ total ${total.toFixed(2)} débil para Over 3.5: máximo WATCH`,12);
 }
 if(k==="under15"){
   if(total>2.40)return cap("NO BET",`λ total ${total.toFixed(2)} demasiado alto para Under 1.5`,22);
   if(total>2.05&&!exceptional(.10))return cap("WATCH",`λ total ${total.toFixed(2)} alto para Under 1.5: máximo WATCH`,9);
 }
 if(k==="under25"){
   if(total>3.15)return cap("NO BET",`λ total ${total.toFixed(2)} incompatible con Under 2.5`,24);
   if(total>2.80&&!exceptional(.10))return cap("WATCH",`λ total ${total.toFixed(2)} alto para Under 2.5: máximo WATCH`,10);
 }
 if(k==="under35"){
   if(total>4.05)return cap("NO BET",`λ total ${total.toFixed(2)} incompatible con Under 3.5`,24);
   if(total>3.60&&!exceptional(.10))return cap("WATCH",`λ total ${total.toFixed(2)} alto para Under 3.5: máximo WATCH`,10);
 }
 if(k==="bttsYes"){
   if(minL<.75)return cap("NO BET",`λ mínimo ${minL.toFixed(2)} demasiado bajo para BTTS Sí`,24);
   if((minL<1.00||total<2.10)&&!exceptional(.10))return cap("WATCH",`BTTS Sí sin soporte λ suficiente (mín. ${minL.toFixed(2)}, total ${total.toFixed(2)}): máximo WATCH`,10);
 }
 if(k==="bttsNo"){
   if(minL>=1.35)return cap("NO BET",`Ambos λ son altos (mín. ${minL.toFixed(2)}) para BTTS No`,24);
   if(minL>=1.10&&total>=2.55&&!exceptional(.10))return cap("WATCH",`BTTS No contradice un perfil ofensivo alto (mín. ${minL.toFixed(2)}, total ${total.toFixed(2)}): máximo WATCH`,10);
 }
 return {penalty:0,maxDecision:null,reason:"",structural:false};
}
function historicalMarketReliability(k){
 const map={away15:.70,bttsYes:.95,away05:.90,over25:1.08,awayDnb:1.08,homeDnb:1.05,awayOrDraw:1.05,homeOrDraw:1.05};
 return map[String(k)]??1;
}
function rankingScore(row,opts={}){
 // V2.0: Score base + penalizaciones de coherencia estructural antes de premiar el precio.
 ...
 const mc=marketLambdaCoherence(row,opts);row.marketLambdaCoherence=mc;
 const rel=historicalMarketReliability(row.market);row.historicalReliability=rel;
 const histPenalty=clamp((1-rel)*30,0,15);
 return clamp(base-c.penalty-mc.penalty-histPenalty,0,100);
}
...
function selectorEligibility(row,opts={}){
 ...
 if(Number(row.odds)<1.35)return {ok:false,ratio,maxDecision:"NO BET",reason:"Cuota <1.35: disponible para combinadas, no elegible como pick individual"};
 if(row.market==="away15"&&!(Number(opts.al)>=1.50&&p>=.42&&conf>=82&&rob>=80&&worst>=.12))return {ok:false,ratio,maxDecision:"WATCH",reason:"V2: Visitante Over 1.5 exige λ≥1.50, P≥42%, Conf≥82, Rob≥80 y EV peor≥12%"};
 if(row.baseDecision!=="VALUE BET"&&row.dec!=="VALUE BET")return {ok:false,ratio,reason:"No supera filtros base"};

 // Build 2000 · coherencia direccional primero, precio después.
 // Aplica a ML, DNB, doble oportunidad y hándicaps direccionales.
 const lc=lambdaContradiction(row,opts);
 if(lc.opposes&&lc.gap>=.30)return {ok:false,ratio,maxDecision:"NO BET",reason:`Contradicción estructural con λ (${lc.gap.toFixed(2)}): NO BET aunque el EV sea positivo`};
 if(lc.opposes&&lc.gap>=.15)return {ok:false,ratio,maxDecision:"WATCH",reason:`Contradice la dirección del λ por ${lc.gap.toFixed(2)}: máximo WATCH`};
 if(lc.opposes&&lc.gap>0 && !(conf>=85&&rob>=80&&worst>=.10))return {ok:false,ratio,maxDecision:"WATCH",reason:`Contradicción leve con λ (${lc.gap.toFixed(2)}): exige Conf≥85, Rob≥80 y EV peor≥10%`};

 // Build 2000 · coherencia específica por mercado para goles y ambos marcan.
 const mc=marketLambdaCoherence(row,{...opts,hl:opts.hl,al:opts.al});
 if(mc.maxDecision)return {ok:false,ratio,maxDecision:mc.maxDecision,reason:mc.reason};
 ...
 return {ok:true,ratio,reason:(opts.matchMode==="cup"&&opts.cupLeg==="first")?"Elegible · Copa ida: Conf selector -5":(lc.opposes?`Elegible · excepción de coherencia λ (${lc.gap.toFixed(2)})`:"Elegible")};
}
function topGeneralQuality(row){
 ...
 if(score<78)return {ok:false,reason:"Score < 78"};
 if(conf<80)return {ok:false,reason:"Confianza < 80"};
 if(rob<75)return {ok:false,reason:"Robustez < 75"};
 if(worst<.05)return {ok:false,reason:"EV peor < 5%"};
 if(Number(row.odds)>=3 && !(rob>=80&&worst>=.10))return {ok:false,reason:"Cuota ≥3.00 exige Rob≥80 y EV peor≥10%"};
 return {ok:true,reason:"Cumple mínimos TOP GENERAL"};
}
function rankRows(rows,opts={}){
 // V2.0: primero elegibilidad dura; luego Score. Cada categoría siempre muestra su mejor candidato relativo.
 rows.forEach(r=>{
   r.category=marketCategory(r);r.baseDecision=r.dec;
   const sel=selectorEligibility(r,opts);r.marketDivergence=sel.ratio;r.selectorReason=sel.reason;r.selectorEligible=sel.ok;r.decisionCap=sel.maxDecision||null;
 ...
 // V2.0: 1X ≡ H1X2 Local +1 y X2 ≡ H1X2 Visitante +1. Solo la mejor cuota puede competir como recomendación.
 ...
 for(const cat of cats){
   ...
   group.forEach(r=>{
     if(r.decisionCap==="NO BET")r.dec="NO BET";
     else if(r.baseDecision==="VALUE BET")r.dec="WATCH";
   ...
   if(eligible.length){
     ...
     if(top.decisionCap==="NO BET")top.dec="NO BET";
     else if(top.baseDecision==="VALUE BET")top.dec="WATCH";
 ...
 if(categoryWinners.length){
   // V2.0: un TOP GENERAL no es obligatorio. Además de ser VALUE BET elegible,
...
let eg=expGoals(hd,ad,+document.getElementById("leagueAvg").value||1.35,absHS,absAS,{mode:matchMode,homeLeagueAvg:+document.getElementById("homeLeagueAvg").value||1.35,awayLeagueAvg:+document.getElementById("awayLeagueAvg").value||1.35,homeLeagueStrength:Number(document.getElementById("homeLeagueStrength").value),awayLeagueStrength:Number(document.getElementById("awayLeagueStrength").value),homeTeamStrength:Number(document.getElementById("homeTeamStrengthLeague")?.value??document.getElementById("homeTeamStrength").value),awayTeamStrength:Number(document.getElementById("awayTeamStrengthLeague")?.value??document.getElementById("awayTeamStrength").value),leagueHomeGoalsAvg:Number(document.getElementById("leagueHomeGoalsAvg")?.value)||null,leagueAwayGoalsAvg:Number(document.getElementById("leagueAwayGoalsAvg")?.value)||null,leagueHomeXgAvg:Number(document.getElementById("leagueHomeXgAvg")?.value)||null,leagueAwayXgAvg:Number(document.getElementById("leagueAwayXgAvg")?.value)||null,cupContext}),hl=eg.hl,al=eg.al,ps=probs(hl,al),dataConf=confidenceScore(hd,ad,absH,absA),rows=[];
...
let d=r.diagnostics;
let h=`<div class="card result-capture-card"><div class="result-head"><h2>🎯 Resultado V2.5.0</h2><button type="button" class="download-result-btn" onclick="downloadResultCapture()">📸 Descargar resultado</button></div>
...
function exportData(){let b=new Blob([JSON.stringify(loadDB(),null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="SoyJordan_Picks_V2_0_Backup_Legacy.json";a.click();document.getElementById("dataMsg").textContent="Backup V2.0 exportado."}
...
function refreshComboPickOptions(){
  ...
  // V2.0 build 1800: en combinadas se muestran TODOS los picks guardados
...
function exportData(){
  const payload={format:"SoyJordan Picks Backup",version:"2.0",exportedAt:new Date().toISOString(),analyses:loadDB(),bets:loadBets(),combos:loadCombos(),bank:loadBank()};
  const b=new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}),a=document.createElement("a");a.href=URL.createObjectURL(b);a.download="SoyJordan_Picks_V2_0_Backup.json";a.click();document.getElementById("dataMsg").textContent="Backup V2.0 exportado.";
}
...
async function buildResultCanvas(r){
  ...
  drawText(ctx,'V2.5',W-pad-24,y+55,21,'700','#fff','right');
  y+=brandH+24;
  ...
  if(generalPick){
    ...
    drawText(ctx,'Ningún candidato supera Score 78 · Conf. 80 · Rob. 75 · EV peor ≥5%.',pad+20,y+93,15,'600','#555');
  }
  y+=topGeneralH+20;
  ...
  // ---------- Footer ----------
  y+=26;
  drawText(ctx,'Motor λ: V2.5 · Selector jerárquico: V2.5 · BUILD 2500',W/2,y+18,15,'700','#555','center');
...
function evaluateMarketOutcome(row,r,hg,ag){
  ...
  switch(k){
    ...
    case "homeU05": return hg<0.5?"win":"loss";
    case "home15": return hg>1.5?"win":"loss";
    case "homeU15": return hg<1.5?"win":"loss";
    case "home25": return hg>2.5?"win":"loss";
    case "homeU25": return hg<2.5?"win":"loss";
    case "away05": return ag>0.5?"win":"loss";
    case "awayU05": return ag<0.5?"win":"loss";
    case "away15": return ag>1.5?"win":"loss";
    case "awayU15": return ag<1.5?"win":"loss";
    case "away25": return ag>2.5?"win":"loss";
    case "awayU25": return ag<2.5?"win":"loss";
...
if("serviceWorker"in navigator&&location.protocol!=="file:")navigator.serviceWorker.register("sw.js?ver=2.5-build2500").catch(()=>{});



(function(){
const test={
 meta:{date:"2026-08-15",league:"Liga de Prueba",season:"2026/27",leagueAvg:"1.50",dataSource:"Datos ficticios",home:"Atlético Aurora",away:"Deportivo Central",notes:"PRUEBA V2.5.0 build 2500 — datos ficticios."},
...
function loadTest(){
 ...
 document.getElementById("vp-test-status").textContent="✅ Prueba V2.5.0 build 2500 cargada (stake visual calibrado: WATCH frágil = SEGUIMIENTO; NO BET = —).";
}
...
// ==================== V2.0 · STAKE / BAJAS / FILTROS / COMBINADAS ====================
...
function recommendedStake(row,isTopGeneral=false){
  ...
  if(!row||!['WATCH','VALUE BET'].includes(decision))return {units:0,amount:0,mode:'no-bet',tier:'NO BET'};
  const odds=Number(row.odds)||0,worst=Number(row.worstEV)||0,ev=Number(row.e)||0,edge=Number(row.ed)||0,conf=Number(row.conf)||0,rob=Number(row.robustness)||0,score=Number(row.rankScore)||0,p=Number(row.p)||0,rel=historicalMarketReliability(row.market);
  if(odds<=1||worst<=0)return {units:0,amount:0,mode:decision==='WATCH'?'watch-only':'no-bet',tier:'SEGUIMIENTO'};
  let units=1;
  if(conf>=82&&rob>=78&&worst>=.08)units=3;
  else if(conf>=76&&rob>=70&&worst>=.04)units=2;
  if(conf>=86&&rob>=84&&worst>=.12)units=4;
  if(isTopGeneral)units=5;
  const noOutlier=!row.dataOutlier;
  const superTop=isTopGeneral&&score>=95&&conf>=94&&rob>=94&&worst>=.20&&ev>=.25&&edge>=.10&&p>=.65&&odds>=1.45&&odds<=2.20&&rel>=1&&noOutlier&&!row.lambdaContradiction?.opposes&&!row.marketLambdaCoherence?.structural;
  if(superTop)units=10;
  if(row.dataOutlier&&units>4&&units<10)units=4;
  // Cuotas altas reducen la categoría de riesgo, sin bajar de 1u cuando sigue siendo apuesta.
  const factor=odds<1.80?1:odds<2.20?.90:odds<2.80?.75:odds<3.50?.60:.40;
  if(units<10)units=Math.max(1,Math.round(units*factor));
  if(rel<.85&&units>1)units=Math.max(1,units-1);
  const bank=Math.max(0,Number(financeSummary()?.current)||Number(loadBank()?.initial)||0),unitValue=bank*.01;
  return {units,amount:unitValue*units,unitValue,mode:'bet',tier:units===10?'SUPER TOP':units===5?'TOP PREMIUM':`${units}U`};
}
...
function enhanceStakeDisplay(r){
  ...
  if(general){
    ...
    if(card&&!card.querySelector('.stake-reco')){const d=document.createElement('div');d.className='stake-reco';d.innerHTML=`<b>💰 STAKE RECOMENDADO: ${stakeLabel(general,true)}</b><br><span>Escala V2: 1u–5u según calidad; 10u solo SUPER TOP. 1u = 1% del bank.</span>`;card.appendChild(d)}
  }
  const cats=['GANADORES','GOLES','HANDICAP','AMBOS MARCAN'];
...
function absenceCalibration(side){
  ...
  return {participation:val('AbsParticipation',100),tactical:val('AbsTactical',100),replacement:val('AbsReplacement',0),certainty:val('AbsCertainty',100),absorption:val('AbsAbsorption',0)};
}
function calibratedAbsenceValue(raw,c){
  ...
  const absorption=1-(clampImpact(c.absorption)/100);
  return clampImpact(saturated*participation*tactical*replacement*certainty*absorption);
}
renderAbsenceCards=function(){
  const wrap=document.getElementById('absenceCards');if(!wrap)return;wrap.innerHTML='';
  for(const [side,title] of [['home','🏠 Local'],['away','✈️ Visitante']]){
    wrap.innerHTML+=`<div class="card absence-cal-card"><h3>${title}</h3><div class="grid"><label>Impacto bruto ataque 0–100<input id="${side}AbsAtt" class="abs-aggregate" type="number" min="0" max="100" step="1" value="0"></label><label>Impacto bruto defensa 0–100<input id="${side}AbsDef" class="abs-aggregate" type="number" min="0" max="100" step="1" value="0"></label><label>Participación / titularidad<input id="${side}AbsParticipation" class="abs-calibration" type="number" min="0" max="100" value="100"></label><label>Peso táctico<input id="${side}AbsTactical" class="abs-calibration" type="number" min="0" max="100" value="100"></label><label>Calidad del reemplazo<input id="${side}AbsReplacement" class="abs-calibration" type="number" min="0" max="100" value="0"></label><label>Certeza de la baja<input id="${side}AbsCertainty" class="abs-calibration" type="number" min="0" max="100" value="100"></label><label>Absorción histórica 0–100 <span class="muted">(opcional)</span><input id="${side}AbsAbsorption" class="abs-calibration" type="number" min="0" max="100" value="0"></label></div><div id="${side}AbsSummary" class="absence-summary"></div></div>`;
  }
...
restoreDraft=function(){__restoreDraft1800();try{const d=JSON.parse(localStorage.getItem(DRAFT_KEY)||'null');if(!d)return;for(const side of ['home','away']){if(d.absenceRawInput?.[side])setAggregateAbsence(side,d.absenceRawInput[side].att,d.absenceRawInput[side].def);const c=d.absenceCalibration?.[side];if(c){for(const [suf,key] of [['AbsParticipation','participation'],['AbsTactical','tactical'],['AbsReplacement','replacement'],['AbsCertainty','certainty'],['AbsAbsorption','absorption']]){const el=document.getElementById(side+suf);if(el)el.value=c[key]}}}document.querySelector('.abs-calibration')?.dispatchEvent(new Event('input',{bubbles:true}))}catch(e){}};
...
// Build 2000 · combinadas integradas en Historial, sin sección independiente.
