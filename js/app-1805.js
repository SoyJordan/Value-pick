...
async function buildResultCanvas(r){
  ...
  if(generalPick){
    ...
    drawText(ctx,`💰 STAKE RECOMENDADO: ${gs.units.toFixed(2)}u${gs.amount>0?` · ${money(gs.amount)}`:''}`,pad+20,y+120,17,'900','#166534');
  }else{
  ...
  // ---------- Footer ----------
  y+=26;
  drawText(ctx,'Motor λ: V1.7.1 · Selector jerárquico: V1.8.0 · BUILD 1805',W/2,y+18,15,'700','#555','center');
...
function loadTest(){
 ...
 document.getElementById("vp-test-status").textContent="✅ Prueba V1.8.0 build 1805 cargada (WATCH frágil = solo seguimiento; stake conservador calibrado).";
}
...
function stakeLabel(row,isTop=false){
  ...
  if(s.mode==='watch-only')return 'SOLO SEGUIMIENTO';
...
function enhanceStakeDisplay(r){
  ...
  if(general){
    ...
    if(card&&!card.querySelector('.stake-reco')){const d=document.createElement('div');d.className='stake-reco';d.style.cssText='margin-top:10px;padding:10px 12px;border:2px solid #166534;border-radius:10px;background:#eaf7ee;font-size:16px';d.innerHTML=`<b>💰 STAKE RECOMENDADO: ${stakeLabel(general,true)}</b><br><span>¼ Kelly conservador sobre EV peor, ajustado por Confianza, Robustez y penalización de cuota alta. 1u = 1% del bank.</span>`;card.appendChild(d)}
  }
  const cats=['GANADORES','GOLES','HANDICAP','AMBOS MARCAN'];
