...
- Backup actualizado a V1.6.2.9.1 y compatibilidad con datos V1.6.1.
...
## V1.6.2.9 — Auditoría postpartido
...
## V1.6.2.9
- Limpieza total de referencias antiguas de versión.
- `<title>` verificado como `SoyJordan Picks V1.6.2.9`.
- Marcador visible `build 1629`.
- Conserva los arreglos de V1.6.2.7:
  - xG real acepta `2.51` y `2,51` en iPhone.
  - columnas Local/Visitante de la captura movidas hacia la izquierda.
  - service worker con navegación `network-first`.
- Motor matemático V1.6 sin cambios.


## V1.6.2.9 — Corrección del motor de ausencias
- Corrige la saturación frecuente de impacto agregado en 100/100.
- La importancia 0–100 sigue siendo individual por jugador.
- Confirmada = 100% de su importancia; duda = 50%; disponible = 0%.
- El agregado ya no suma linealmente jugadores. Usa rendimientos decrecientes: la ausencia más importante fija la base y las demás consumen gradualmente el margen restante.
- Conserva el límite de ±10% que las ausencias pueden aplicar al λ ofensivo/defensivo.
- No modifica las estadísticas, Poisson, value, robustez ni la auditoría postpartido.
